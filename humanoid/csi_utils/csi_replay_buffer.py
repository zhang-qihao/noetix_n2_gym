...
class CSIReplayBuffer:
    """Fixed-size buffer to store experience tuples."""

    def __init__(self, obs_dim, obs_horizon, num_labels, buffer_size, device):
        """Initialize a ReplayBuffer object.
        Arguments:
            buffer_size (int): maximum size of buffer
        """
        self.state_buf = torch.zeros(buffer_size, obs_horizon, obs_dim).to(device)
        self.label_buf = torch.zeros(buffer_size, num_labels).to(device)
    ...
    def insert(self, state_buf, label_buf):
        ...
        if end_idx > self.buffer_size:
            ...
            self.label_buf[self.step:self.buffer_size] = label_buf[:self.buffer_size - self.step]
            self.label_buf[:end_idx - self.buffer_size] = label_buf[self.buffer_size - self.step:]
        else:
            self.state_buf[start_idx:end_idx] = state_buf
            self.label_buf[start_idx:end_idx] = label_buf
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        for _ in range(num_mini_batch):
            ...
            yield self.state_buf[sample_idxs, :].to(self.device), self.label_buf[sample_idxs, :].to(self.device)
