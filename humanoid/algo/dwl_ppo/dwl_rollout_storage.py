...
class DWLRolloutStorage:
    ...
    def _save_hidden_states(self, hidden_states):
        ...
        hid = hidden_states[0] if isinstance(hidden_states[0], tuple) else (hidden_states[0],)
        # initialize if needed
        if self.saved_hidden_states is None:
            self.saved_hidden_states = [
                torch.zeros(self.observations.shape[0], *hid[i].shape, device=self.device) for i in range(len(hid))
            ]
        # copy the states
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        if self.privileged_observations is not None:
            ...
            padded_next_privileged_obs_trajectories, _ = split_and_pad_trajectories(self.next_privileged_observations, self.dones)
        else:
            padded_privileged_obs_trajectories = padded_obs_trajectories
            padded_next_privileged_obs_trajectories = padded_obs_trajectories
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                next_privileged_obs_batch = padded_next_privileged_obs_trajectories[:, first_traj:last_traj]
                ...
                yield obs_batch, privileged_obs_batch, actions_batch, next_privileged_obs_batch, values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    hid_batch,
