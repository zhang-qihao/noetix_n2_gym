...
from humanoid.algo.dwl_ppo.dwl_ppo import DWL_PPO
from humanoid.algo.dwl_ppo.dwl_actor_critic import DWLActorCritic
...
class DWLOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] == "DWL_PPO":
        ...
        policy: DWLActorCritic = policy_class(
        ...
        self.alg: DWL_PPO | Distillation = alg_class(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    obs, privileged_obs, rewards, dones, infos, termination_ids, termination_privileged_obs = self.env.step(actions.to(self.env.device))
                    # Move to device
                    ...
                    # process the step
                    self.alg.process_env_step(rewards, dones, infos, next_privileged_obs)
