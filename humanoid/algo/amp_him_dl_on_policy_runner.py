...
class AMPHIMDLOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        amp_expert_data: MotionLoaderNing = self.env.motion_loader
        self.amp_state_normalizer = Normalizer(self.env.motion_loader.observation_dim, self.device)

        normalize_style_reward = self.discriminator_cfg.pop("normalize_style_reward")
        if normalize_style_reward:
            self.amp_style_reward_normalizer = Normalizer(1, self.device)  
        else: self.amp_style_reward_normalizer = None
    ...
    def train_mode(self):
        ...
        if self.empirical_normalization:
            ...
            self.amp_state_normalizer.train()
            self.amp_style_reward_normalizer.train()
    ...
    def eval_mode(self):
        ...
        if self.empirical_normalization:
            ...
            self.amp_state_normalizer.eval()
            self.amp_style_reward_normalizer.eval()
