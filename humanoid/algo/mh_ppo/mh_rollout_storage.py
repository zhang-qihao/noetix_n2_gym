...
class MHRolloutStorage:
    ...
    def __init__(
        ...
        if privileged_obs_shape is not None:
            ...
            self.privileged_observations = None
            self.next_privileged_observations = None
        self.rewards = torch.zeros(num_transitions_per_env, num_envs, num_rew_fn, device=self.device)
        ...
        if training_type == "rl":
            self.values = torch.zeros(num_transitions_per_env, num_envs, num_rew_fn, device=self.device)
            self.actions_log_prob = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
            self.mu = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
            self.sigma = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
            self.returns = torch.zeros(num_transitions_per_env, num_envs, num_rew_fn, device=self.device)
    ...
    def compute_returns(self, last_values, gamma, lam, normalize_advantage: bool = True):
        ...
        tot_advantages = self.returns - self.values
        self.advantages = tot_advantages.sum(dim=-1)
