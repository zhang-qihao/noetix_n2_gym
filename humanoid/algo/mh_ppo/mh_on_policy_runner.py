...
from humanoid.algo.mh_ppo.mh_ppo import MH_PPO

class MHOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] == "MH_PPO":
        ...
        policy: HIMActorCritic = policy_class(
            num_obs, num_privileged_obs, self.env.num_actions, critic_output_dim=self.env.num_rew_fn,**self.policy_cfg
        ).to(self.device)
        ...
        self.alg: MH_PPO | Distillation = alg_class(policy,
                                                         device=self.device, 
        ...
        self.empirical_normalization = self.cfg["runner"]["empirical_normalization"]
        ...
        self.git_status_repos = [humanoid.__file__]
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    obs, privileged_obs, rewards, dones, infos, termination_ids, \
                        termination_privileged_obs, _ = self.env.step(actions.to(self.env.device))
                    # Move to device
                    ...
                    # process the step
                    self.alg.process_env_step(rewards_stored, dones, infos, next_privileged_obs)
                    ...
                    if self.log_dir is not None:
                        ...
                        # Update rewards
                        if self.alg.rnd:
                            cur_ereward_sum += rewards.view(self.env.num_envs, self.env.num_rew_fn).sum(dim=-1)
                            cur_ireward_sum += intrinsic_rewards  # type: ignore
                            cur_reward_sum += rewards + intrinsic_rewards
                        else:
                            cur_reward_sum += rewards.view(self.env.num_envs, self.env.num_rew_fn).sum(dim=-1)
    ...
    def train_mode(self):
        ...
        # -- HIM
        self.alg.policy.estimator.train()
        # -- RND
        if self.alg.rnd:
            self.alg.rnd.train()
        # -- Normalization
